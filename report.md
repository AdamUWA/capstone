...
OCR integrated document preprocessing is a well established standard technique for dealing with poor quality text based data. Given that the system was required to handle scanned copies of the original reports as the default input format, OCR was the natural choice [1]. The process and implementation specifics were inspired by the [Docling project](https://www.docling.ai/). Additional choices for the system such as pre-chunking and serializing the documents into JSON ojbect file (see Figure A.1 of the Appendix) for efficient vector store loading were influenced by Auer et al from their [Docling Technical Report](https://arxiv.org/abs/2408.09869) [7]. System dependent document preprocessing times with OCR are exhibited in Table 1.
